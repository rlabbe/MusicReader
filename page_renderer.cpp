...
void PageRenderer::goto_virtual_page(int virtual_index)

{
    ...
    // Map virtual index to physical page and segment
    ...
    for (int page = 1; page <= total; ++page) {
        ...
        for (int seg = 0; seg < seg_count; ++seg) {
            ...
            if (virtual_count == virtual_index) {

                current_physical_page_ = page;

                current_segment_index_ = seg;
...
std::vector<std::string> PageRenderer::get_all_page_displays() const
{
    ...
    for (int page = 1; page <= total; ++page) {
        int seg_count = segment_count(page);
        for (int seg = 0; seg < seg_count; ++seg) {
            displays.push_back(format_page_display(page, seg));
        }
    }
