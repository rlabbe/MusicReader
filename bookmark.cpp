...
std::string to_json(std::vector<Bookmark>& bookmarks) noexcept
{
    try {
        ...
        std::function<nlohmann::json(const std::vector<Bookmark>&, int)> convert =
            [&](const std::vector<Bookmark>& bmarks, int level) {
                nlohmann::json arr = nlohmann::json::array();
                for (const auto& b : bmarks) {
                    nlohmann::json obj;
                    obj["title"] = b.title_;
                    if (b.page_num_.has_value())
                        obj["page_num"] = b.page_num_.value();
