...
public:
    ...
    void set_memory_usage(const std::string& msg) { memory_usage_->setText(QString::fromStdString(msg)); }
    ...
    void update_style()
    {
        ...
        QString label_style = QString("QLabel { background-color: %1; border: 2px solid %2; border-style: ridge; "
                                      "padding: 2px; margin-right: 5px; }")
                                  .arg(base_color, shadow_color);

        QString combo_style =
            QString("QComboBox { background-color: %1; border: 1px solid %2; padding: 2px; margin-right: 5px; }")
                .arg(base_color, shadow_color);
